...
// setInterval(getSysUpdate, 30000); // 30s for debug
    ...
    <Gtk.MenuButton class="unset" visible={icon((i) => i.length > 0)}>
      ...
      <Gtk.Popover hasArrow={false}>
        <Gtk.Box orientation={VERTICAL}>
          <Gtk.Box orientation={HORIZONTAL}>
            ...
            >
              <Gtk.Button
                ...
                css="border-radius: 200px;margin-right: 8px;"
                tooltipText="Update"
          ...
          <Gtk.ScrolledWindow
            class="updates"
            windowPlacement={Gtk.CornerType.TOP_LEFT}
            hscrollbarPolicy={Gtk.PolicyType.NEVER}
            vscrollbarPolicy={Gtk.PolicyType.AUTOMATIC}
            maxContentHeight={680}
            propagateNaturalHeight={true}
          >
            <Gtk.ListBox class="packages" valign={START}>
              <For each={update}>
                {(p) => (
